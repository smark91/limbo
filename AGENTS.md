...
9. **Service Worker / PWA Caching**: When modifying frontend static assets (HTML, CSS, JavaScript, SVGs) listed in the `ASSETS` array in [sw.js](file:///docker_config/stacks/limbo/app/frontend/sw.js), you **MUST** increment the `CACHE_NAME` constant (e.g., bump `'limbo-cache-v3'` to `'limbo-cache-v4'`). Bumping this string is what triggers the browser to install the updated worker, cache the new files, and display the update prompt to active users.
